...
public class Cartelera {
    ...
    // Método para buscar películas por género específico
    public List<Pelicula> filtrarPorGenero(String genero) {
        List<Pelicula> peliculas = new ArrayList<>();
        String sql = "SELECT id, titulo, director, anio, duracion, genero FROM Cartelera WHERE genero = ?";
    ...
    public List<Pelicula> filtrarPorRangoAnios(int anioInicio, int anioFin) {
        List<Pelicula> peliculas = new ArrayList<>();
        // La consulta SQL selecciona las películas donde 'anio' es mayor o igual al inicio Y menor o igual al fin.
        String sql = "SELECT id, titulo, director, anio, duracion, genero FROM Cartelera WHERE anio >= ? AND anio <= ?";

        try (Connection conn = DBConector.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {

            pstmt.setInt(1, anioInicio);
            pstmt.setInt(2, anioFin);
            ...
            System.err.println("❌ Error al filtrar por rango de años: " + e.getMessage());
        }
        return peliculas;
    }
}